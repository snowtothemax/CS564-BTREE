...
void manySearchKeyTest() {
  {
    ...
    try {
      ...
      std::cout << "Test 5 failed: Bad scan range from startScan()"
                << std::endl;
    ...
    while (1) {
      try {
        ...
        std::cout << "Test 5 failed: scan was not initialized when it should "
                     "have been"
                  << std::endl;
    ...
    if (count != 5000) {
      std::cout << "Test 5 failed: Did not scan the correct number of records"
                << std::endl;
    }
