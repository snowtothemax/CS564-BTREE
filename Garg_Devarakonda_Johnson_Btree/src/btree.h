...
/*
 *Small struct used to pass pairs of key ands pages
 *
 */
struct KeyPagePair {
  int key;
...
class BTreeIndex {
 private:
  ...
  /**
   * @brief Insert key-value in leaf(assumes space esists)
   *
   * @param key - key to add
   * @param rid - value to add
   * @param currNode - node to insert into
   */
  void simpleLeafInsert(int key, const RecordId rid, LeafNodeInt *currNode);

  /**
   * @brief Insert key-value in internal node(assumes space esists)
   *
   * @param key - key to add
   * @param pageId- value to add
   * @param currNode - node to insert into
   */
 ...
 public:
   ...
   */
  KeyPagePair recursiveInsert(int key, const RecordId rid, const bool isLeaf,
                              PageId currPage);
